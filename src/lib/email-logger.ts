...
class EmailLogger {
  ...
  constructor() {
    ...
    if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS || !Number.isFinite(parsedPort)) {
      const error = 'Отсутствуют или некорректны SMTP_HOST/SMTP_PORT/SMTP_USER/SMTP_PASS';
  ...
  private buildHtmlTemplate(subject: string, message: string) {
    ...
    const formattedMessage = message
      .split(/\n\s*/)
      .map((line) => `<p style="margin: 0 0 12px; color: #1b1e27;">${escapeHtml(line)}</p>`)
      .join('');
          ...
          <table role="presentation" style="max-width: 640px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 6px 18px rgba(0,0,0,0.08); padding: 32px;">
            <tr>
              <td>
                ...
                <h1 style="margin: 0 0 16px; font-size: 22px; color: #0f172a;">${escapeHtml(
                  subject,
                )}</h1>
